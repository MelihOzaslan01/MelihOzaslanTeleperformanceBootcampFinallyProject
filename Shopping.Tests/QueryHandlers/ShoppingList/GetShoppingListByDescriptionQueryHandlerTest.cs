...
using Shopping.Application.ShoppingListCQRS.Handlers.QueryHandlers.GetShoppingListByDescription;
...
public class GetShoppingListByDescriptionQueryHandlerTest
{
    [Fact]
    public async void GetShoppingListByDescription_IsSuccess()
    {
        var getShoppingListByDescriptionQueryRequest = new Bogus.Faker<GetShoppingListByDescriptionQueryRequest>();
        ...
        var command = new GetShoppingListByDescriptionQueryHandler(mockShoppingListRepository.Object,mockProductRepository.Object,mapper.Object);
        
        var response = await command.Handle(getShoppingListByDescriptionQueryRequest.Generate(), CancellationToken.None);

        Assert.True(response.IsSuccess);
        Assert.NotNull(response.ShoppingList);
    }
    [Fact]
    public async void GetShoppingListByDescription_IsFailure()
    {
        var getShoppingListByDescriptionQueryRequest = new Bogus.Faker<GetShoppingListByDescriptionQueryRequest>();
